    ...
    <div className="flex flex-wrap items-center gap-2 bg-slate-900/60 border border-slate-800 p-2 rounded-xl shrink-0 select-none">
      <div className="flex items-center gap-1">
        ...
        <select
          ...
          className="bg-transparent text-slate-205 text-xs font-bold outline-none cursor-pointer pr-2 focus:text-indigo-300 transition-colors border-none"
        >
          {options.map((opt) => (
            <option 
              key={opt.value} 
      ...
      {value === 'custom' && (
        <div className="flex items-center gap-2 border-l border-slate-800 pl-2 animate-fade-in">
          <input
            ...
            className="bg-slate-950 border border-slate-800 text-slate-200 text-xs rounded px-2 py-0.5 outline-none focus:border-indigo-500 cursor-pointer"
          />
